...
- CI: GitHub Actions (`.github/workflows/e2e.yml`) runs the same suite on `ubuntu-latest` using Google Chrome + matching chromedriver (Chrome for Testing). Browser/chromedriver paths can be overridden via `CHROMIUM_BIN`/`CHROMEDRIVER_BIN` env vars (see `e2e/tests/conftest.py`).
